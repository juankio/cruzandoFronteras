<template>
    <div>
        <section class="bg-white dark:bg-gray-800/0 text-gray-900 dark:text-white py-12">
            <div class="container mx-auto px-4">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <!-- Misión -->
                    <div
                        class="relative p-6 bg-white dark:bg-gray-900 rounded-lg shadow-lg transform transition-transform duration-300 hover:z-10 hover:scale-105 hover:-translate-y-2">
                        <div class="top-line bg-red-600 dark:bg-pink-500 w-full h-1 absolute top-0 left-0"></div>
                        <h3 class="text-2xl font-semibold mb-4 text-red-600 dark:text-pink-500">Nuestra Misión</h3>
                        <p class="text-base text-gray-800 dark:text-gray-300">
                            Acompañar a comunidades en condiciones de vulnerabilidad, ofreciéndoles servicios de
                            <UiUnderlinetext word="educación" Tool="Programas educativos para un futuro mejor" />,
                            <UiUnderlinetext word="recreación" Tool="Actividades para el bienestar físico y mental" /> y
                            <UiUnderlinetext word="bienestar" Tool="Enfoque en salud integral" />
                            como pilar fundamental del modo de intervención, acompañados de
                            <UiUnderlinetext word="servicios de nutrición" Tool="Acceso a alimentos saludables" />,
                            <UiUnderlinetext word="salud" Tool="Atención médica y preventiva" />,
                            buscando la formación en valores y motivando a sus integrantes a ser personas
                            activas.
                        </p>
                    </div>
                    <!-- Visión -->
                    <div
                        class="relative p-6 bg-white dark:bg-gray-900 rounded-lg shadow-lg transform transition-transform duration-300 hover:z-10 hover:scale-105 hover:-translate-y-2">
                        <div class="top-line bg-red-600 dark:bg-pink-500 w-full h-1 absolute top-0 left-0"></div>
                        <h3 class="text-2xl font-semibold mb-4 text-red-600 dark:text-pink-500">Nuestra Visión</h3>
                        <p class="text-base text-gray-800 dark:text-gray-300">
                            La FUNDACIÓN PARA EL MEJORAMIENTO INTEGRAL DE LA CALIDAD DE VIDA DE LOS COLOMBIANOS
                            “CRUZANDO FRONTERAS” será
                            <UiUnderlinetext word="líder a nivel nacional" Tool="Excelencia en iniciativas sociales" />
                            en el desarrollo de actividades que
                            conlleven al
                            <UiUnderlinetext word="mejoramiento de la calidad de vida"
                                Tool="Impulsando vidas mejores" />
                            de los colombianos mediante la promoción y
                            ejecución de actividades y programas que mejoren las
                            <UiUnderlinetext word="relaciones interpersonales"
                                Tool="Fortaleciendo lazos comunitarios" /> para la
                            <UiUnderlinetext word="convivencia ciudadana" Tool="Fomentando la paz social" /> y el
                            progreso de las comunidades en general.
                        </p>
                    </div>
                    <!-- Valores -->
                    <div
                        class="relative p-6 bg-white dark:bg-gray-900 rounded-lg shadow-lg transform transition-transform duration-300 hover:z-10 hover:scale-105 hover:-translate-y-2">
                        <div class="top-line bg-red-600 dark:bg-pink-500 w-full h-1 absolute top-0 left-0"></div>
                        <h3 class="text-2xl font-semibold mb-4 text-red-600 dark:text-pink-500">Áreas de Impacto</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-800 dark:text-gray-300">
                            <div>
                                <ul class="list-disc pl-5">
                                    <li
                                        class="fade-in delay-6 transition-transform duration-300 hover:scale-105 hover:-translate-y-2 text-red-600 dark:text-pink-500">
                                        <p class="text-gray-800 dark:text-gray-300">Niñez</p>
                                    </li>
                                    <li
                                        class="fade-in delay-6 transition-transform duration-300 hover:scale-105 hover:-translate-y-2 text-red-600 dark:text-pink-500">
                                        <p class="text-gray-800 dark:text-gray-300">Juventud</p>
                                    </li>
                                    <li
                                        class="fade-in delay-6 transition-transform duration-300 hover:scale-105 hover:-translate-y-2 text-red-600 dark:text-pink-500">
                                        <p class="text-gray-800 dark:text-gray-300">Adulto Mayor</p>
                                    </li>
                                    <li
                                        class="fade-in delay-6 transition-transform duration-300 hover:scale-105 hover:-translate-y-2 text-red-600 dark:text-pink-500">
                                        <p class="text-gray-800 dark:text-gray-300">Diversidad Funcional</p>
                                    </li>
                                    <li
                                        class="fade-in delay-6 transition-transform duration-300 hover:scale-105 hover:-translate-y-2 text-red-600 dark:text-pink-500">
                                        <p class="text-gray-800 dark:text-gray-300">Grupos Étnicos</p>
                                    </li>
                                    <li
                                        class="fade-in delay-6 transition-transform duration-300 hover:scale-105 hover:-translate-y-2 text-red-600 dark:text-pink-500">
                                        <p class="text-gray-800 dark:text-gray-300">Población de Alto Riesgo</p>
                                    </li>
                                    <li
                                        class="fade-in delay-6 transition-transform duration-300 hover:scale-105 hover:-translate-y-2 text-red-600 dark:text-pink-500">
                                        <p class="text-gray-800 dark:text-gray-300">Desplazados</p>
                                    </li>
                                    <li
                                        class="fade-in delay-6 transition-transform duration-300 hover:scale-105 hover:-translate-y-2 text-red-600 dark:text-pink-500">
                                        <p class="text-gray-800 dark:text-gray-300">Mujeres Cabeza de Hogar</p>
                                    </li>
                                </ul>
                            </div>
                            <div>
                                <p>Contribuir de manera efectiva al mejoramiento de la calidad de vida de los
                                    colombianos en sus diversas poblaciones.</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
    </div>
</template>

<script setup>
</script>